...
def rearrange(img):
	...
	# if img_shape[1] % 2 == 1:
	# 	img_shape[1] -= 1
	img_ = np.zeros(img_shape, img.dtype)
	xh, yh = int(img_shape[1]/2), int(img_shape[0]/2)
	...
	# img_[0:yh,0:xh], img_[yh:img_shape[0],xh:img_shape[1]] = img[yh:img_shape[0],xh:img_shape[1]], img[0:yh,0:xh]
	# img_[0:yh,xh:img_shape[1]], img_[yh:img_shape[0],0:xh] = img[yh:img_shape[0],0:xh], img[0:yh,xh:img_shape[1]]

	img_[0:yh,0:xh] = img[yh:img_shape[0],xh:img_shape[1]]
	img_[yh:img_shape[0], xh:img_shape[1]] = img[0:yh,0:xh]

	img_[0:yh,xh:img_shape[1]] = img[yh:img_shape[0],0:xh] 
	img_[yh:img_shape[0],0:xh] = img[0:yh,xh:img_shape[1]]
	return img_
...
class KCFTracker:
	...
	def getFeatures(self, image, inithann, scale_adjust=1.0):
		...
		self._tmpl_sz = list(map(int, self._tmpl_sz))
